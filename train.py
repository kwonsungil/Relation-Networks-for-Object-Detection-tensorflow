...
from models.losslayer import RPN_loss
from utils import pascal_voc as pascl
from utils.predict_loss import Predict_loss
...
class Solver(object):   
 ...
 # training process       
    def train_model(self, backbone='resnet'):
        ...
        #resnet
            variables_to_restore = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="resnet_v2_101")
        ...
        with tf.Session() as sess:
            ...
            # 기존 rcnn restore
            # print('restore from : ', os.path.join(cfg.OUTPUT_DIR))
            # saver.restore(sess, tf.train.latest_checkpoint(os.path.join(cfg.OUTPUT_DIR)))
            for step in range(self.max_iter+1):
                ...
                # a,b,c,d,e,f = sess.run([self.net.rois_positive_score, self.net.rpn_bbx,\
                #                                     self.net.image_width, self.net.image_height, self.net.all_anchors, self.net.net_feature], feed_dict=feed_dict)
                # print(a.shape)
                # print(b.shape)
                # print('anchors : ', e.shape)
                # print(f.shape)
                # print(c, d)
