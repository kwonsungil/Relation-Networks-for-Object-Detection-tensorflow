...
from utils.anchor_generate import all_anchor_conner
from utils.anchor_label import anchor_labels_process
from utils.rois_target import proposal_target

slim = tf.contrib.slim
import numpy as np
import tfplot as tfp
# from models .resnet import resnet_v2_101
from models import resnet
from core.relation_module import RelationModule

"""build the whole network"""
class Net(object):
    def __init__(self, is_training=True, keep_prob=0.5, backbone='resnet'):
        ...
        if self.is_training:
            if backbone == 'vgg16':
                self.weight_file_path = cfg.vgg_weight_path
            else:
                self.weight_file_path = cfg.resnet_weight_path
        else:
            self.weight_file_path = cfg.weigt_output_path

        self.build_network(backbone)
    ...
    def build_network(self, backbone):
        if backbone=='vgg16':
            self.net_feature = self.vgg16(self.image) #get the vggl6 feature
        else:
            with slim.arg_scope(resnet.resnet_arg_scope()):
                # self.net_feature = resnet.resnet_v2_101(self.image, is_training=False)
                self.net_feature = resnet.resnet_v2_101(self.image, is_training=True)

        self.rois_output = self.rpn_net(self.net_feature, self.num_anchor) #rpn net
        print('self.rois_output : ', self.rois_output)
        ...
        if self.is_training:
            with tf.variable_scope('roi_process') :
                self.rois_coord, rois_score_process, scores_roi = self.rois_process(self.rois_positive_score, self.rpn_bbx, \
        ...
        pool5, bboxes = self._crop_pool_layer(self.net_feature, self.rois_coord, "pool5")  #roi pooling layer
        print('pool5 : ', pool5)
        print('bboxes : ', bboxes)
        # pool5 = pool5[0]

        # fc7 = self._head_to_tail(pool5, self.is_training)
        # fc7 = self._head_to_tail_base(pool5)
        fc7 = self._head_to_tail_relation(pool5, bboxes)
    ...
    def _head_to_tail_relation(self, pool5, bboxes):
        # with tf.variable_scope('vgg_16'):
        pool5_flat = slim.flatten(pool5, scope='flatten')
        print('pool5_flat: ', pool5_flat)
        pool5_flat.set_shape([cfg.anchor_batch, 7 * 7 * 1024])
        bboxes.set_shape([cfg.anchor_batch, 4])
        # pool5_flat.set_shape([cfg.test_max_rpn_nms_num, 7 * 7 * 1024])
        # bboxes.set_shape([cfg.test_max_rpn_nms_num, 4])
        fc6 = slim.fully_connected(pool5_flat, 1024, scope='fc6')
        rm_1 = RelationModule(fc6, bboxes)
        # 128, 1024
        rm_1.result.set_shape([None, 1024])
        fc7 = slim.fully_connected(rm_1.result, 1024, scope='fc7')
        rm_2 = RelationModule(fc7, bboxes)
        return rm_2.result
...
#        acnhor_h = tf.cast(anchors[:,3] - anchors[:,1]+1.0, tf.float32)
    ...
    def _crop_pool_layer(self, bottom, rois, name): 
        ...
        return slim.max_pool2d(crops, [2, 2], padding='SAME'), bboxes
